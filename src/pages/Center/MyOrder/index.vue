          ...
          <tr v-for="(item, index) in order.orderDetailList" :key="item.id">
            ...
            <template v-if="index===0">
              ...
              <td :rowspan="order.orderDetailList.length" width="8%" class="center">
                <a href="#" class="btn">{{order.processStatus === 'UNPAID' ? '未完成' : '已支付'}} </a>
              </td>
              <td :rowspan="order.orderDetailList.length" width="13%" class="center">
                <ul class="unstyled">
                  <li>
...
export default {
  ...
  methods: {
    async getMyOrder(pageNo=1){
      this.page = pageNo
      const {page , limit} = this
      const result = await this.$API.reqMyOrder(page,limit)
      if(result.code === 200){
