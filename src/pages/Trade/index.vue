<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix " v-for="(item, index) in tradeList.userAddressList" :key="item.id">
        <span class="username" :class="{selected : item===selectedAddr }">{{item.consignee}}</span>
        <p @click="selectedAddr = item">
      ...
      <div class="detail">
        <h5>商品清单</h5>
        <ul class="list clearFix" v-for="(trade, index) in tradeList.detailArrayList" :key="trade.skuId">
          <li>
...
<script>
  ...
  export default {
    ...
    watch: {
      'tradeList.userAddressList'(value){
        const defaultAddress = value.find((item => item.isDefault === '1'))
        if(defaultAddress){
          this.selectedAddr = defaultAddress
        }
      }
    },
  }
